...
def _load_and_preprocess(args):
    ...
    try:
        img = cv2.imread(str(path))
        if img is None:
            return idx, None, f"Failed to load: {path}"
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_pil = Image.fromarray(img)
        img_processed = preprocess(img_pil, desired_size=resolution)
        return idx, np.array(img_processed, dtype=np.float32), None
    except Exception as e:
        return idx, None, f"{type(e).__name__}: {str(e)}"


def img_to_hdf5_parallel(cxr_paths: List[Union[str, Path]], out_filepath: str, resolution=320, num_workers=None, chunk_size=100):
    """
    Convert directory of images into a .h5 file using multiprocessing.

    Args:
        cxr_paths: List of paths to images
        out_filepath: Output H5 file path
        resolution: Target resolution
        num_workers: Number of parallel workers (default: cpu_count)
        chunk_size: Number of images to buffer before writing to H5
    """
    ...
    args_list = [(idx, path, resolution) for idx, path in enumerate(cxr_paths)]

    print(f"Processing {dset_size} images with {num_workers} workers...")

    # Create H5 file
    with h5py.File(out_filepath, 'w') as h5f:
        img_dset = h5f.create_dataset('cxr', shape=(dset_size, resolution, resolution), dtype=np.float32)
        ...
        with Pool(num_workers) as pool:
            # Use imap_unordered for better performance (results come as completed)
            for idx, img_data, error in pool.imap_unordered(_load_and_preprocess, args_list, chunksize=32):
                if error:
                    failed_images.append((idx, str(cxr_paths[idx]), error))
                elif img_data is not None:
                    write_buffer[idx] = img_data

                pbar.update(1)

                # Write buffer to H5 when it reaches chunk_size
                if len(write_buffer) >= chunk_size:
                    for buf_idx, buf_data in sorted(write_buffer.items()):
                        img_dset[buf_idx] = buf_data
                        written_count += 1
                    write_buffer.clear()
                    pbar.set_postfix({'written': written_count, 'failed': len(failed_images)})
    ...
    # Summary
    print(f"\nCompleted: {written_count}/{dset_size} images written to {out_filepath}")
    if failed_images:
        print(f"Failed: {len(failed_images)} images")
        for idx, path, err in failed_images[:5]:
            print(f"  [{idx}] {path}: {err}")
        if len(failed_images) > 5:
            print(f"  ... and {len(failed_images) - 5} more")

    return {'written': written_count, 'failed': len(failed_images), 'failed_images': failed_images}
